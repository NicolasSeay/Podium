...
public class AuthServiceImpl implements AuthService {
    ...
    public Map<String, Object> register(String email, String password, String firstName, String lastName) { if (email == null || password == null) throw error(HttpStatus.BAD_REQUEST, "email and password are required"); if (users.findByEmail(email).isPresent()) throw error(HttpStatus.CONFLICT, "email is already registered"); return response(users.save(new User(null, email, password, firstName, lastName))); }
    ...
    public User currentUser(String authorization, String userHeader) { String userId = authorization == null ? userHeader : tokens.get(bearer(authorization)); if (userId == null) throw error(HttpStatus.UNAUTHORIZED, "authentication is required"); return users.findById(Long.valueOf(userId)).orElseThrow(() -> error(HttpStatus.UNAUTHORIZED, "unknown user")); }
    private Map<String, Object> response(User user) { String token = id(); tokens.put(token, String.valueOf(user.id())); return Map.of("user", user, "token", token); }
