...
@Service public class VehicleServiceImpl implements VehicleService {
 ...
 public Vehicle create(Long userId,Map<String,Object>b){return vehicles.save(new Vehicle(null,userId,required(b,"name"),text(b,"make",null),text(b,"model",null),integer(b,"year",null)));}
 public Vehicle update(Long userId,Long id,Map<String,Object>b){Vehicle c=get(userId,id);return vehicles.save(new Vehicle(c.id(),c.userId(),text(b,"name",c.name()),text(b,"make",c.make()),text(b,"model",c.model()),integer(b,"year",c.year())));}
